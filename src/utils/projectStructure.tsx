...
const TreeNode: React.FC<TreeNodeProps> = ({ node }) => {
    ...
    const filenameArray = node.path.split('/');
    const filename = filenameArray.slice(5).join("/")
    const filenameHighlightNode = filenameArray.slice(4).join("/")
    ...
    return (
        <Box>
            <Box direction="row" align="center" gap="small"  onClick={handleToggle}>
                ...
                {node.type==="file" && <Box align='center' direction='row' onMouseEnter={highlightNode} onMouseLeave={resetNode} onClick={singleClickMouse} onDoubleClick={doubleClickMouse}><Document style={{paddingRight:'3px'}} size='small' color="black"/><Button><Text wordBreak='keep-all' size='medium' weight='lighter' color='#515A5A'>{node.name}</Text></Button></Box>}
